...
router.get("/callback", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to exchange token", details: error.message });
  }
});
...
router.post("/save-tasks", async (req, res) => {
  ...
  if (!tagId || !Array.isArray(tasks) || tasks.length === 0) {
    console.error(
      "Validation failed. Received tagId:",
      tagId,
      "and tasks:",
      tasks
    );
    return res.status(400).json({
      success: false,
      error:
        "Invalid tag ID or tasks data. Ensure 'tagId' is provided and 'tasks' is a non-empty array.",
    });
  }
  ...
  try {
      ...
      if (task.summary && task.summary.includes("-")) {
        ...
        if (DepartmentName) {
          ...
          if (users.length > 0) {
            ...
            console.log(
              "No users found in the department. Setting assignedTo to null."
            );
          }
        } else {
    ...
    const formattedTasks = tasks.map((task) => ({
      ...
      startDate: task.start?.dateTime
        ? new Date(task.start.dateTime)
        : new Date(),
    ...
    let existingTaskDoc = await TaskGroup.findOne({ tagId });

    if (existingTaskDoc) {
      existingTaskDoc.TaskGroup.push(...formattedTasks);
      ...
      const newTaskDoc = new TaskGroup({ tagId, Tasks: formattedTasks });
...
router.post("/taskdetail", async (req, res) => {
  ...
  try {
    ...
    const taskDocuments = await TaskGroup.find({
...
router.put("/updateTask", async (req, res) => {
  ...
  try {
    const updatedDoc = await TaskGroup.findOneAndUpdate(
      { "tasks._id": id }, // lowercase 'tasks'
