  ...
  try {
    ...
    if (file.deficiencies && file.deficiencies.length > 0) {
      for (let i = 0; i < file.deficiencies.length; i++) {
        ...
        const tagText =
          item.Tag || item.tag || item.tagId || item.TagId || `Tag ${i + 1}`;
        ...
        if (
          ...
          if (typeof solution === "string") {
            ...
            // Extract meaningful string instead of JSON
    ...
    doc.on("end", () => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=POC_Report_Debug.pdf`
      );
      res.send(finalBuffer);
    });
    const pageWidth =
      doc.page.width - doc.page.margins.left - doc.page.margins.right;
    const colWidths = [80, 200, 80, pageWidth - 80 - 200 - 80];
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("CMS-2567 Deficiency and Plan of Correction Report (DEBUG)", {
        align: "center",
      });
    ...
    for (let i = 0; i < allDeficiencies.length; i++) {
      ...
      const rowHeight = Math.min(Math.max(...heights, 30), 600); // Cap height at 600
