...
const Adduser = () => {
  ...
  useEffect(() => {
    axios
      .get("https://dpp-backend.vercel.app/api/users/User123")
      .then((response) => {
  ...
  <img src="/assets/logo-dpp1.png" alt="Logo" className="h-8 w-auto" />
      ...
      <div className="lg:ml-64 p-4 sm:p-8 w-full">
        ...
        <div className="w-full sm:w-3/4 mx-auto text-center mb-10">
          <h3
            className="font-bold text-black-800 opacity-80 text-2xl sm:text-2xl lg:text-3xl"
            style={{ fontFamily: "Plus Jakarta Sans, sans-serif" }}
          >
            Add User
          </h3>
          <p
            className="text-gray-900 text-sm sm:text-base lg:text-md leading-[25.2px] opacity-40 mb-4"
            style={{ fontFamily: "Plus Jakarta Sans, sans-serif" }}
          >
          ...
          <table className="min-w-full text-left border-collapse border border-[#F2F2F2]">
            <thead>
              <tr className="bg-gray-100 text-black border-b border-[#F2F2F2]">
                <th className="px-6 py-6 text-left font-sm font-semibold">
                  User ID
                </th>
                <th className="px-6 py-4 text-left font-sm font-semibold">
                  Date
                </th>
                <th className="px-6 py-4 text-left  font-sm font-semibold">
                  User Name
            ...
            <tbody>
              {[...new Map(users.map((user) => [user._id, user])).values()] // Remove duplicates
                ...
                .map((user, index) => (
                  ...
                  >
                    <td className="py-4 px-6 text-blue-700 font-sm">
                      #{user._id}
                    </td>
                    <td className="py-4 px-6 text-gray-500 font-sm">
                      {user.createdAt}
                    </td>
                    <td className="py-4 px-6 text-gray-500 font-sm">
                      {user.firstname}
